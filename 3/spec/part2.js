(function() {
  ...
  describe('Part II', function() {
    ...
    describe('every', function() {
      ...
      it('passes for a collection of all-truthy results without an iterator passed in', function() {
        expect(_.every([true, {}, 1])).to.be.true;
      });
      ...
      it('fails for a collection of all-falsy results without an iterator passed in', function() {
        expect(_.every([null, 0, undefined])).to.be.false;
      });
    ...
    describe('once', function() {
      ...
      it('should only run a user-defined function if it has not been run before', function() {
        ...
        var onceIncrement = _.once(increment);
      ...
      it('should apply arguments to the user-defined function', function() {
        ...
        var onceAdd = _.once(add);

        expect(onceAdd(1, 2, 3)).to.equal(6);
      });
      ...
      it('should return the result of the first call for every subsequent call', function() {
        ...
        expect(onceAdd(1, 2, 3)).to.equal(6);
        expect(onceAdd(4, 5, 6)).to.equal(6);
        expect(onceAdd(7, 8, 9)).to.equal(6);
      });
    });
    ...
    describe('memoize', function() {
      ...
      it('should return a stored result from a memoized function, after function is run once', function() {
      ...
      it('should memoize a function with several arguments', function() {
        var sumAll = function() {
        ...
        var memoSumAll = _.memoize(sumAll);

        var randomLength = Math.floor(Math.random() * 8) + 2;
        var randomArgs = Array.from({length: randomLength}, function() {
          return Math.floor(Math.random() * 10);
        });

        var actual = memoSumAll.apply(null, randomArgs);
        var expected = randomArgs.reduce(function(sum, num) {
          return sum + num;
        });
    ...
    describe('delay', function() {
      ...
      it('should have successfully passed function arguments in', function() {
        ...
        _.delay(callback, 1000, ...randomArgsOne);
        _.delay(callback, 1000, ...randomArgsTwo);

        // add check for premature invocation to callback
        clock.tick(900);

        expect(callback).to.have.not.been.called;
        // end check for premature invocation

        clock.tick(1000);

        expect(callback).to.have.been.calledWith(...randomArgsOne);
        expect(callback).to.have.been.calledWith(...randomArgsTwo);
      });
    });
    ...
    describe('shuffle', function() {
      ...
      it('should not be in the same order as the original object', function() {
        // Technically, this test can fail if the items are shuffled into the same order
        // that they started in. By using a large array here, the chances of randomly shuffling
        // the items into the original order, or the same order twice in a row, become very small.
        var veryLargeArray = Array(100).fill().map(function(empty, index) { return index; });
        var previousResult = veryLargeArray;
        for (var i = 0; i < 3; i++) {
          var result = _.shuffle(veryLargeArray);
          expect(result).to.not.eql(previousResult);
