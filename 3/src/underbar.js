(function() {
  ...
  _.reject = function(collection, test) {
    ...
    // give native error if use forEach
  ...
  _.uniq = function(array, isSorted, iterator) {
    ...
    _.each(array, function(item) {
      if (iterator === undefined) {
        ...
        if (!transformedValue.includes(iterator(item))) {
          output.push(item);
          transformedValue.push(iterator(item));
        }
      }
    });
  ...
  _.map = function(collection, iterator) {
    ...
    if (Array.isArray(collection)) {
      for (var i = 0; i < collection.length; i++) {
        var value = iterator(collection[i], i, collection);
        output.push(value);
      }
    } else {
      for (var prop in collection) {
        var value = iterator(collection[prop]);
        output.push(value);
      }
    }
    return output;
  ...
  _.every = function(collection, iterator) {
    ...
    if (iterator === undefined) {
      return _.reduce(collection, function(acc, ele) {
        if (!ele) {
          acc = false;
        }
        return acc;
  ...
  _.memoize = function(func) {
    ...
    return function () {
      ...
      if (argus in cache) {
        ...
        return (cache[argus] = func.apply(this, argus));
        // cache[output] = result;
        // return result;
      }
    };
  ...
  _.shuffle = function(array) {
    //copy array
    //for lop push array.length times of random item
    //get random
    //delete random
    //push random
    ...
    var copyed = array.slice(0);
    var output = [];
    for (var i = 0; i < array.length; i++) {
      var index = getRandom(copyed.length);
      output.push(copyed[index]);
